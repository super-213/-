...
class VehicleManager:
    ...
    def remove_vehicle(self, license_plate):
        ...
        if result:
            slot_id = result[0][0]
            # 删除车辆记录
            query = """DELETE FROM vehicles WHERE license_plate = %s"""
            self.db.execute_update(query, (license_plate,))
            # 释放车位
            self.db.execute_update("UPDATE parking_slots SET state = 0 WHERE slot_id = %s", (slot_id,))
            print(f"车辆 {license_plate} 已离开，车位 {slot_id} 已释放")
        else:
...
def main():
    ...
    while True:
        ...
        if user_input == 'q':
            ...
            license_plate = input("请输入车牌号：")
            vehicle_manager = VehicleManager(db)
            vehicle_manager.remove_vehicle(license_plate)
